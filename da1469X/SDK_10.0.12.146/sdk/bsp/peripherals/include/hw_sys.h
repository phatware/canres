 ...
 * defines how long the RSTN or the POR-configured GPIO pin should
 * be asserted in order to trigger a POR.
 *
 * \param [in] time time in 4096 * RC32K period units. Should be less
 *                  than 128 (~16.2 sec).
 *
 * \note In case of triggering a POR from a GPIO, hw_gpio_configure_por_pin()
 * should also be used.
 *
 * \note Setting \b time to 0 disables POR generation completely (even for RSTN).
 *
