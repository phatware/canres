...
typedef struct {
        ...
        uint8_t  packet_size;           /**< The size of the packet*/
        uint8_t  packet_size_txfill;    /**< Packet size of TX buffer. */
...
/**
 * \brief Send Remote-Wakeup Signal to Host.
 *
 */
void hw_usb_request_remote_wakeup(void);
...
/**
 * \brief Set the USB HW-Block in SUSPENDED mode.
 *        This is used when the Remote Wakeup signaling
 *        is triggered so the USB HW block will process
 *        the RESUME-EOP and generate the RESUME event
 *        interrupt
 *
 */
void hw_usb_set_hw_mode_to_suspended(void);
 ...
 *             The application can get the USB state through the
 *             emUSB API call USBD_GetState() and/or the registered
 *             emUSB callback through USBD_RegisterSCHook(...);
 *             See the emUSB documentation for details.
 */
DEPRECATED_MSG("API no longer supported, use the emUSB API call USBD_GetState() to get the USB state.")
bool hw_usb_is_suspended(void);

/**
 * \brief Set flag to indicate USB device is suspended
 *
 * \deprecated This function is deprecated. User should not call this function.
 *             A USB device cannot suspend on its own. The USB host must request it.
 *             The USB state is handled internally and the Application can get it
 *             through the emUSB API call USBD_GetState() and/or the registered
 *             emUSB callback through USBD_RegisterSCHook(...);
 *             See the emUSB documentation for details.
 */
DEPRECATED_MSG("API no longer supported, the USB state is handled internally and the call of this function has no effect.")
