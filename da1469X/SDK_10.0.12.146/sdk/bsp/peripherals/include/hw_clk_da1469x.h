 ...
 */
#define XTALRDY_CYCLES_TO_LP_CLK_CYCLES(x, lp_freq) ((((uint32_t)(x)) * lp_freq + dg_configRC32M_FREQ_MIN/(125) - 1) / (dg_configRC32M_FREQ_MIN/125))
...
{
        return (REG_GETF(CRG_XTAL, XTALRDY_STAT_REG, XTALRDY_COUNT) == 0 && REG_GETF(CRG_XTAL, XTAL32M_STAT1_REG, XTAL32M_STATE) == 0x8 /*XTAL_RUN state*/);
}
 ...
 *
 * \warning The effect of this function is overruled if hw_gpio_configure_por_pin()
 * has previously been called with a valid pin configuration.
 *
 */
...
{
        REG_SETF(CRG_TOP, CLK_XTAL32K_REG, XTAL32K_DISABLE_AMPREG, dg_configEXT_LP_IS_DIGITAL);
}
...
__STATIC_INLINE void hw_clk_set_sysclk(sys_clk_is_t mode)
{
        ...
        if (mode == SYS_CLK_IS_XTAL32M && REG_GETF(CRG_TOP, CLK_CTRL_REG, RUNNING_AT_RC32M)) {
                ...
                ASSERT_ERROR(hw_clk_is_xtalm_started()); // DO NOT SWITCH TO XTAL32M IF IS NOT SETTLED
...
__STATIC_FORCEINLINE void hw_clk_pll_sys_on(void)
{
        ...
        /* Now turn on PLL. */
        REG_SET_BIT(CRG_XTAL, PLL_SYS_CTRL1_REG, PLL_SEL_MIN_CUR_INT);
        REG_SET_BIT(CRG_XTAL, PLL_SYS_CTRL2_REG, PLL_RECALIB);
        ...
        REG_CLR_BIT(CRG_XTAL, PLL_SYS_CTRL2_REG, PLL_RECALIB);
        hw_clk_delay_usec(5);
...
/**
 * \brief Calculate the system clock frequency for the current system clock
 *
