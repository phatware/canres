...
typedef enum {
        ...
        HW_GPIO_CLK_RC32K_FUNC = 1,          /**< Map clock RC32K to GPIO function FUNC_CLOCK */
        ...
        HW_GPIO_CLK_RC32M_FUNC = 4,          /**< Map clock RC32M to GPIO function FUNC_CLOCK */
        HW_GPIO_CLK_DIVN_FUNC = 5,           /**< Map clock DIVn to GPIO function FUNC_CLOCK */
...
typedef enum {
        ...
        HW_GPIO_CLK_RC32K_OUT = GPIO_GPIO_CLK_SEL_REG_RC32K_OUTPUT_EN_Msk,          /**< Map clock RC32K to dedicated GPIO */
        ...
        HW_GPIO_CLK_RC32M_OUT = GPIO_GPIO_CLK_SEL_REG_RC32M_OUTPUT_EN_Msk,          /**< Map clock RC32M to dedicated GPIO */
 ...
 *
 * XTAL32M is mapped to GPIO P0_12
 * RC32M is mapped to GPIO P0_13
 * XTAL32K is mapped to GPIO P0_14
 * DIVN is mapped to GPIO P0_15
 * RCX is mapped to GPIO P0_16
 * RC32K is mapped to GPIO P0_17
 *
...
{
        ASSERT_WARNING((clk == HW_GPIO_CLK_XTAL32K_OUT) || (clk == HW_GPIO_CLK_RC32K_OUT) ||
                (clk == HW_GPIO_CLK_RCX_OUT) || (clk == HW_GPIO_CLK_XTAL32M_OUT) ||
                (clk == HW_GPIO_CLK_RC32M_OUT) || (clk == HW_GPIO_CLK_DIVN_OUT));
...
{
        ASSERT_WARNING((clk >= HW_GPIO_CLK_XTAL32K_FUNC) || (clk < HW_GPIO_CLK_LAST_FUNC));
 ...
 * a POR. (The RSTN pin can still be used though.)
 *
 * \note By selecting a valid GPIO pin, RC32K is kept enabled (hw_clk_disable_rc32k() has no effect),
 * unless hw_sys_enable_clockless() has previously been called (hibernation mode). This is because
 * triggering a POR from GPIO requires RC32K to be enabled. For the same reason, in hibernation mode
 * (i.e. if hw_sys_enable_clockless() has previously been called and the system has entered sleep
 * state) no POR can be triggered using the selected GPIO pin (only the RSTN pin can be used, since
 * it causes a HW_RESET, which in turn enables RC32K).
 *
 * \sa hw_sys_set_por_timer()
 */
__STATIC_INLINE void hw_gpio_configure_por_pin(HW_GPIO_PORT port, HW_GPIO_PIN pin, bool polarity)
{
        unsigned pin_select = 0x3F;
        if (port == HW_GPIO_PORT_0) {
