 ...
 */
#define ASCII_3080        0x33303830      // '3' '0' '8' '0'
#define ASCII_2522        0x32353232      // '2' '5' '2' '2'
...
/* Function to apply D2522 preferred settings */
static void hw_sys_set_preferred_values_d2522(HW_PD pd)
{
        switch (pd) {
        case HW_PD_AON:
        ...
        case HW_PD_TMR:
                ...
                RAW_SET_MASKED(0x50010040, 0x00C00000, 0x00C00000);
...
/* Function to apply D3080 preferred settings */
static void hw_sys_set_preferred_values_d3080(HW_PD pd)
{
        // NOTE: Revisit the following cases when CircePort preferred settings are available!!
        switch (pd) {
        case HW_PD_AON:
                REG_SET_MASKED(CRG_TOP, BANDGAP_REG, 0x00001000, 0x00001020);
                CRG_TOP->BIAS_VREF_SEL_REG = 0x000000CA;
                ...
                REG_SET_MASKED(CRG_TOP, CLK_XTAL32K_REG, 0x0000007E, 0x00000024);
                break;
        case HW_PD_SYS:
                REG_SET_MASKED(CHARGER, CHARGER_CTRL_REG, 0x00000C00, 0x003F6A78);
                REG_SET_MASKED(CHARGER, CHARGER_PWR_UP_TIMER_REG, 0x000003FF, 0x00000002);
        ...
        case HW_PD_TMR:
                REG_SET_MASKED(CRG_XTAL, CLK_FREQ_TRIM_REG, 0x3FF00000, 0x000AFD70);
                REG_SET_MASKED(CRG_XTAL, TRIM_CTRL_REG, 0x000000C0, 0x00000562);
                REG_SET_MASKED(CRG_XTAL, XTAL32M_CTRL0_REG, 0x1FC38002, 0x0C01E6B6);
                REG_SET_MASKED(CRG_XTAL, XTAL32M_CTRL1_REG, 0x707FFF00, 0x0500A1A4);
                REG_SET_MASKED(CRG_XTAL, XTAL32M_CTRL2_REG, 0x00000FFF, 0x001E45C4);
                REG_SET_MASKED(CRG_XTAL, XTAL32M_CTRL3_REG, 0x40000000, 0x40096255);
                RAW_SET_MASKED(0x50010040, 0x00C00000, 0x00C00000);
                REG_SET_MASKED(CRG_XTAL, XTALRDY_CTRL_REG, 0x000000FF, 0x00000180);
...
void hw_sys_set_preferred_values(HW_PD pd)
{
        ...
        if (hw_sys_device_info_check(DEVICE_CHIP_ID_MASK, DEVICE_CHIP_ID_2522)) {
                /* D2522AB */
                hw_sys_set_preferred_values_d2522(pd);
        } else if (hw_sys_device_info_check(DEVICE_CHIP_ID_MASK, DEVICE_CHIP_ID_3080)) {
                /* D3080AA */
                hw_sys_set_preferred_values_d3080(pd);
        }
}
...
static void hw_sys_pll_locktime_tune(void)
{
        ...
        uint32_t power_ctrl_reg = CRG_TOP->POWER_CTRL_REG;
        ...
        *(volatile uint32_t *)0x50020B2C |= 0x1UL;
        ...
        CRG_TOP->POWER_CTRL_REG = power_ctrl_reg;
...
static uint32_t get_device_variant(void)
{
        ...
        if ((size == 1) && (variant != NULL)) {
                return MAKE_DEVICE_VARIANT_BITFIELD(variant[0]);
        }
...
bool hw_sys_device_info_init(void)
{
        ...
        uint32_t revision = MAKE_DEVICE_REVISION_BITFIELD((REG_GETF(CHIP_VERSION, CHIP_REVISION_REG, CHIP_REVISION) - 'A'));
        uint32_t step = MAKE_DEVICE_STEP_BITFIELD((REG_GETF(CHIP_VERSION, CHIP_TEST1_REG, CHIP_LAYOUT_REVISION) - 'A'));
        ...
        case ASCII_3080:
                device_chip_id.value = DEVICE_CHIP_ID_3080;
                break;
        case ASCII_2522:
                device_chip_id.value  = DEVICE_CHIP_ID_2522;
                break;
        default:
                return false;
        }
        ...
        if ((revision < DEVICE_REVISION_MIN) || (revision > DEVICE_REVISION_MAX) ||
            ...
            (__builtin_popcount(revision) != 1) || (__builtin_popcount(step) != 1)) {
                return false;
        }

        hw_sys_device_info_data = DA1469X | device_chip_id.value | revision | step;
...
bool hw_sys_device_info_check(uint32_t mask, uint32_t attribute)
{
        ...
        ASSERT_WARNING(attribute_masked >= DEVICE_INFO_ATTRIBUTE_MIN(mask) &&
                       attribute_masked <= DEVICE_INFO_ATTRIBUTE_MAX(mask) &&
                       (__builtin_popcount(attribute_masked) == 1));

        return((hw_sys_device_info_data & mask) == attribute_masked);
}
