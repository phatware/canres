...
#if (dg_configUSB_DMA_SUPPORT == 1)
static __INLINE void tx_ep_dma_transfer(hw_usb_ep_data_t* ep, uint8_t ep_nr, uint8_t *pd, uint16_t saved)
{
...
__RETAINED_CODE static void tx_fill(uint8_t ep_nr)
{
        ...
        if ((ep_nr != USB_EP_DEFAULT) &&
            ...
            (saved >= 1)) {
                 ...
                 */
                tx_ep_dma_transfer(ep, ep_nr, pd, saved);
        } else
#endif /* dg_configUSB_DMA_SUPPORT */
        {
                volatile uint16_t *txs = ep_regs[ep_nr].txs;
                volatile uint16_t *txd = ep_regs[ep_nr].txd;
                uint16_t remain = saved;
                uint16_t tcount = *txs & TXS_TCOUNT_MASK;
...
static void rx_ep_read(uint8_t ep_nr, bool setup)
{
        ...
        if ((ep_nr != USB_EP_DEFAULT) &&
            (usb_rx_dma.channel_number != HW_DMA_CHANNEL_INVALID)) {
                ...
                if (hw_dma_is_channel_active(usb_rx_dma.channel_number) == HW_DMA_STATE_DISABLED) {
...
#endif /* dg_configUSB_DMA_SUPPORT */
        {
                ...
                ASSERT_ERROR(rxsize <= ep->rx.max_size);
...
void hw_usb_sd3_event(void)
{
        ...
        if (REG_GETF(USB, USB_NFSR_REG, USB_NFS) == HW_USB_NFSR_NODE_OPERATIONAL) {
                 ...
                 * We will restore it on RESUME event */
...
{
        if (REG_GETF(USB, USB_NFSR_REG, USB_NFS) == HW_USB_NFSR_NODE_SUSPEND) {
                ud_nfsr = HW_USB_NFSR_NODE_RESUME;
                REG_SETF(USB, USB_NFSR_REG, USB_NFS, ud_nfsr);
        }
}
...
void hw_usb_resume_event(void)
{
        ...
        ud_nfsr = REG_GETF(USB, USB_NFSR_REG, USB_NFS);

        if (ud_nfsr == HW_USB_NFSR_NODE_SUSPEND || ud_nfsr == HW_USB_NFSR_NODE_RESUME) {
                ud_nfsr = HW_USB_NFSR_NODE_OPERATIONAL;
                REG_SETF(USB, USB_NFSR_REG, USB_NFS, ud_nfsr);
#if (dg_configUSB_SUSPEND_MODE == USB_SUSPEND_MODE_PAUSE)
...
void hw_usb_cfg(const usb_config *cfg)
{
        if (cfg->use_dma) {
                 ...
                 ASSERT_ERROR((cfg->tx_dma_prio <= HW_DMA_PRIO_7 && cfg->tx_dma_prio >= HW_DMA_PRIO_0 ) ||
                              (cfg->rx_dma_prio <= HW_DMA_PRIO_7 && cfg->rx_dma_prio > HW_DMA_PRIO_0 ));
                 ...
                 usb_tx_dma.dma_prio = cfg->tx_dma_prio;
                 ...
                 /* Select the priority of the DMA */
                 usb_rx_dma.dma_prio = cfg->rx_dma_prio;
