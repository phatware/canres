...
extern uint32_t rc32k_clock_hz;

/* in Hz contributes in speeding-down the RC32K */
#define RC32K_NEGATIVE_FREQ_MARGIN_DUE_TO_VOLTAGE      ( 14000 ) /* in Hz the amount of frequency by which the RC32K
                                                                  * could be slowed-down due to voltage change
                                                                  * from 1.2 to 0.7 at -40C due to affect of temperature
                                                                  * on the LDOs.
                                                                  * This can happen only at -40C */

#define RC32K_POSITIVE_FREQ_MARGIN_DUE_TO_VOLTAGE      ( 675 )   /* in Hz the amount of frequency by which the RC32K
                                                                  * could ever be speed-up due to voltage change from
                                                                  * 1.2 to 0.75 at high temperatures (+80C and above)
                                                                  * In this area the RC32K could present up to 1.5Hz/mV
                                                                  * RC32K frequency variation.
                                                                  * Multiplying this by the 450mV worst case of voltage change
                                                                  * between 1.2V and 0.75V we get 675Hz margin*/

/* in Hz contributes in speeding-up or down the RC32K depending on the temperature */
#define RC32K_TEMPERATURE_DRIFT           ( 150 )     /* in Hz/C */

#define RC32K_FREQ_POSITIVE_MARGIN        ( ( RC32K_POSITIVE_FREQ_MARGIN_DUE_TO_VOLTAGE ) + \
                                            ( BANDGAP_TEMPERATURE_CHANGE_FOR_RC32K * RC32K_TEMPERATURE_DRIFT ) )
                                                /* RC32K_POSITIVE_FREQ_MARGIN_DUE_TO_VOLTAGE is due to max 450mV voltage
                                                 * if the RC32K is calibrated when the VDD=1.2V.
                                                 * When temperature is increased, the frequency of the RC32K is increased too.
                                                 * When the voltage is dropped, the frequency of the RC32K is lowered too.
                                                 * BANGAP_TEMP_DRIFT_FOR_RC32K * RC32K_TEMPERATURE_DRIFT is the max
                                                 * possible increase of the RC32K frequency due to temperature
                                                 * change by BANDGAP_TEMPERATURE_CHANGE_FOR_RC32K
                                                 * in sleep before the temperature change is detected */

#define RC32K_FREQ_NEGATIVE_MARGIN        ( ( RC32K_NEGATIVE_FREQ_MARGIN_DUE_TO_VOLTAGE ) + \
                                            ( BANDGAP_TEMPERATURE_CHANGE_FOR_RC32K * RC32K_TEMPERATURE_DRIFT ) )
                                                /* RC32K_NEGATIVE_FREQ_MARGIN_DUE_TO_VOLTAGE is due to max 500mV voltage
                                                 * if the RC32K is calibrated when the VDD=1.2V.
                                                 * When temperature is decreased, the frequency of the RC32K is decreased too.
                                                 * When the voltage is dropped, the frequency of the RC32K is lowered too.
                                                 * BANGAP_TEMP_DRIFT_FOR_RC32K * RC32K_TEMPERATURE_DRIFT is the max
                                                 * possible decrease of the RC32K frequency due to temperature
                                                 * change by BANDGAP_TEMPERATURE_CHANGE_FOR_RC32K
                                                 * in sleep before the temperature change is detected */

/**
 * \brief Value of the minimum RC32K oscillator frequency in Hz
 *        Based on chip characterization data, the lowest RC32K
 *        frequency in a 69x part, can be  23KHz (23000)
 *
 *        \bsp_default_note{\bsp_config_option_app, \bsp_config_option_expert_only}
 */
#define RC32K_FREQ_MIN                         (rc32k_clock_hz - RC32K_FREQ_NEGATIVE_MARGIN)

/**
 * \brief Value of the maximum RC32K oscillator frequency in Hz
 *        Based on chip characterization data, the highest RC32K
 *        frequency in a 69x part, can be  39KHz (39000)
 *
 *        \bsp_default_note{\bsp_config_option_app, \bsp_config_option_expert_only}
 */
#define RC32K_FREQ_MAX                         (rc32k_clock_hz + RC32K_FREQ_POSITIVE_MARGIN)
...
#       elif (dg_configUSE_LP_CLK == LP_CLK_32768) || (dg_configUSE_LP_CLK == LP_CLK_32000)
                ...
                // must be calculated according to dg_configXTAL32K_FREQ and RC32K_FREQ_MAX.
#               define WDOG_VALUE_2_LP_CLKS(wdg_val) ((wdg_val) * dg_configXTAL32K_FREQ / (RC32K_FREQ_MAX / 320))
#       else
...
static __RETAINED_CODE void system_wake_up(void)
{
        ...
        if (system_sleeping == sys_powered_down) {
                 ...
                 * PLL_LOCK_IRQn is assumed to be in NVIC->ICER[1].
                 * If not then the following code must be modified accordingly.
                 */
                ASSERT_WARNING(TIMER2_IRQn < 32 && XTAL32M_RDY_IRQn < 32 && PLL_LOCK_IRQn >= 32);

                NVIC->ICER[0] = iser & ~((uint32_t)(1 << TIMER2_IRQn) | (uint32_t)(1 << XTAL32M_RDY_IRQn));
                NVIC->ICER[1] = iser2 & ~((uint32_t)(1 << (PLL_LOCK_IRQn - 32)));
                ...
                /* If the code stops at this point then the system has gone to sleep with PLL enabled */
                ASSERT_WARNING(hw_clk_check_pll_status() == false)
...
uint8_t pm_get_sys_wakeup_cycles(void)
{
        ...
        /* The wakeup HW FSM is clocked by the RC32K, so we need to convert the
         * cycles needed for the wakeup process to system LP_CLK_Cycles.
         * We are calculating for the slowest possible RC32K case which
         * will take longer time to wakeup */
#if (dg_configUSE_LP_CLK == LP_CLK_32768) || (dg_configUSE_LP_CLK == LP_CLK_32000)
        /* The XTAL32K is always faster than RC32K, so
         * we need to have this conversion and calculate how many XTAL32M
         * cycles will take the wakeup FSM */
        return (wakeup_cycles * dg_configXTAL32K_FREQ  + RC32K_FREQ_MIN - 1) / RC32K_FREQ_MIN;
#elif (dg_configUSE_LP_CLK == LP_CLK_RCX)
        if (rcx_clock_hz >= RC32K_FREQ_MIN) {
                /* In the case where RCX is faster than RC32K
                 * we need to have this conversion and calculate how many RCX
                 * cycles will take the wakeup FSM */
                wakeup_cycles = (wakeup_cycles * rcx_clock_hz + RC32K_FREQ_MIN - 1) / RC32K_FREQ_MIN;
        }
...
__RETAINED_CODE void pm_prepare_sleep(sleep_mode_t sleep_mode)
{
        if (sleep_mode == pm_mode_hibernation || sleep_mode == pm_mode_deep_sleep) {
                ...
                /*
                 * Turn off 1V8 and 1V8P in hibernation, since the system will be woken up by POR,
                 * HW reset or a debounced GPIO trigger, where these power rails are not needed.
                 */
                if (sleep_mode == pm_mode_hibernation) {
