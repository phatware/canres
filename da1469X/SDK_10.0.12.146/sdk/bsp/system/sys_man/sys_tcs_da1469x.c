...
__RETAINED static uint8_t only_m33_size;
__RETAINED static uint8_t only_snc_size;
...
static uint8_t move_entries_at_end(SYS_TCS_GID gid, uint32_t reg)
{
        // sort entries for Group ID
        // keep count of those entries
        uint8_t tcs_entries;
        uint32_t address;
        uint32_t value;
        uint8_t start = tcs_attributes[gid].start;
        uint16_t size = tcs_attributes[gid].size;
        uint8_t index, j;

        tcs_entries = 0;
        for (index = start; index < start + size - tcs_entries; index += 2) {
                if (tcs_data[index] == reg) {
                        tcs_entries += 2;
                        //store value and address that will be moved at the end
                        address = tcs_data[index];
                        value = tcs_data[index + 1];

                        for (j = index; j < start + size - 2; j += 2) {
                                // move all reg pairs by one position
                                tcs_data[j] = tcs_data[j + 2];
                                tcs_data[j + 1] = tcs_data[j + 3];
                        }
                        //store reg pair at the end
                        tcs_data[start + size - 2] = address;
                        tcs_data[start + size - 1] = value;
...
void sys_tcs_get_trim_values_from_cs(void)
{
        ...
        //Move all SYS_TCS_GROUP_PD_PER tcs entries that are only for SNC to the end
#if (dg_configM33_USES_LRA == 0)
        only_snc_size += move_entries_at_end(SYS_TCS_GROUP_PD_PER, (uint32_t)&LRA->LRA_CTRL3_REG);
#endif
#if (dg_configM33_USES_LEDS == 0)
        only_snc_size += move_entries_at_end(SYS_TCS_GROUP_PD_PER, (uint32_t)&PWMLED->PWMLED_CTRL_REG);
#endif

        //Move all SYS_TCS_GROUP_PD_PER tcs entries that are only for M33 to the end
#if (dg_configM33_USES_LRA == 1)
        only_m33_size += move_entries_at_end(SYS_TCS_GROUP_PD_PER, (uint32_t)&LRA->LRA_CTRL3_REG);
#endif
#if  (dg_configM33_USES_LEDS == 1)
        only_m33_size += move_entries_at_end(SYS_TCS_GROUP_PD_PER, (uint32_t)&PWMLED->PWMLED_CTRL_REG);
#endif
}
...
__RETAINED_CODE void sys_tcs_apply_reg_pairs(SYS_TCS_GID gid)
{
        ...
        if (gid == SYS_TCS_GROUP_PD_PER) {
                int both_masters_size;

                both_masters_size = size - (only_m33_size + only_snc_size);
                //reg pairs that can be applied by both masters
                while (both_masters_size > 0) {
...
uint32_t sys_tcs_snc_get_reg_pair_num_of_entries(SYS_TCS_GID gid)
{
        ...
        if (gid == SYS_TCS_GROUP_PD_PER) {
                return ((tcs_attributes[gid].size - only_m33_size) / 2);
        } else {
