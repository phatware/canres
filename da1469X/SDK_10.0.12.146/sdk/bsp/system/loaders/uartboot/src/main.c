...
#define VIRTUAL_BUF_MASK      (0xFFF80000)
 ...
 *
 */
sys_clk_t cm_sysclk = sysclk_PLL96;
...
static bool product_info_helper(uint8_t *info)
{
        ...
        info_size = sprintf(&product_info->str,
                                "PRODUCT INFORMATION:\nDevice classification attributes:\n");

        if (info_size < 0) {
                return false;
        }
        product_info->len = (uint16_t)info_size;

        if (hw_sys_device_info_check(DEVICE_FAMILY_MASK, DA1469X)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "Device family = %s\n", "DA1469x");
        }
        ...
        if (hw_sys_device_info_check(DEVICE_VARIANT_MASK, DA14691)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "Device variant = %s\n", "DA14691");
        }
        else if (hw_sys_device_info_check(DEVICE_VARIANT_MASK, DA14693)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "Device variant = %s\n", "DA14693");
        }
        else if (hw_sys_device_info_check(DEVICE_VARIANT_MASK, DA14695)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "Device variant = %s\n", "DA14695");
        }
        else if (hw_sys_device_info_check(DEVICE_VARIANT_MASK, DA14697)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "Device variant = %s\n", "DA14697");
        }
        else if (hw_sys_device_info_check(DEVICE_VARIANT_MASK, DA14699)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "Device variant = %s\n", "DA14699");
        }
        ...
        if (hw_sys_device_info_check(DEVICE_STEP_MASK, DEVICE_STEP_A)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "%s\n\n", "A");
        }
        else if (hw_sys_device_info_check(DEVICE_STEP_MASK, DEVICE_STEP_B)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "%s\n\n", "B");
        }
        else if (hw_sys_device_info_check(DEVICE_STEP_MASK, DEVICE_STEP_C)) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "%s\n\n", "C");
        }
        ...
        #define TCS_PROD_INFO_LEN       3
        uint32_t *values = NULL;
        uint32_t production_info[TCS_PROD_INFO_LEN] = {0, 0, 0};
        uint8_t size = 0;
        ...
        /* Extract the production package coding that is stored in Byte 10 of the TCS group */
        uint32_t production_package_raw = production_info[2];
        production_package_raw >>= 16;
        ...
        if (production_package_raw == 0x01) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "Package = %s\n", "VFBGA86");
        } else if (production_package_raw == 0x02) {
                info_size = sprintf(&product_info->str + product_info->len,
                                        "Package = %s\n", "VFBGA100");
        }
...
/*
 * Wrapper for writing from RAM buffer to QSPI memory. Verification of written data is performed
 * only if 'read_buf' pointer is not NULL.
 */
static bool qspi_write(uint32_t qspi_addr, const uint8_t *ram_ptr, size_t length, uint8_t *read_buf)
{
        uint32_t len = length;
        int ret;
        ...
        if (qspi_addr >= dg_configQSPI2_FLASH_BASE_ADDR) {
                id = HW_QSPIC2;
        }
#endif

        if (qspi_addr + length > qspi_get_device_size(id)) {
                /* Data will exceed the QSPI FLASH memory size - don't write the data. */
                return false;
        }

        ret = ad_flash_update_possible(qspi_addr, ram_ptr, len);

        if (ret < 0) {
                if (!ad_flash_erase_region(qspi_addr, len)) {
                        return false;
                }
                ret = 0;
        } else {
        ...
        if (ad_flash_write(qspi_addr + ret, ram_ptr + ret, len) != len) {
                return false;
        }
        ...
        if (!ad_flash_read(qspi_addr, read_buf, length)) {
                return false;
        }

        return memcmp(ram_ptr, read_buf, length) == 0;
}
...
static bool cmd_write_ram_to_qspi(HANDLER_OP hop)
{
        ...
        case HOP_EXEC:
                ...
                return qspi_write(hdr->addr, hdr->ptr, hdr->len, (uint8_t *) read_buf_addr);
...
void __attribute__((section("reboot_section"), noinline)) move_to_0_and_boot(void *start, size_t size)
{
        ...
        /*
         * Restore XTAL 32MHz and disable PLL - the rest of the configuration should be cleaned by
         * SW Reset.
         */
...
static bool cmd_direct_write_to_qspi(HANDLER_OP hop)
{
        ...
        case HOP_EXEC:
                ...
                return qspi_write(hdr->addr, cmd_state.data, cmd_state.data_len,
...
static void init(void)
{
        ...
        /* Set VDD to 1.2V - it is required by using PLL */
        REG_SETF(CRG_TOP, POWER_CTRL_REG, VDD_LEVEL, 3);

        if (hw_clk_is_pll_locked() == 0) {
                /* Turn on PLL */
                hw_clk_pll_sys_on();
        }
        ...
        /* Set PLL as sys_clk */
        hw_clk_set_sysclk(SYS_CLK_IS_PLL);
}
