...
#define READ_PIPE_DELAY_0V9             (2)
#define READ_PIPE_DELAY_1V2             (7)
...
bool qspi_get_config(HW_QSPIC_ID id, uint8_t *manufacturer_id, uint8_t *device_type,
                                                                                uint8_t *density)
{
...
__RETAINED_CODE bool qspi_process_operations(void)
{
        ...
        if (qspi_active_op != NULL) {
                ...
                if (qspi_active_op->suspended) {
                        ...
                        qspi_resume(id);
                        // Guard time until RESUME can be issued!
                        hw_clk_delay_usec(QSPI_GET_DEVICE_PARAM(idx, resume_delay_us));
...
__RETAINED_CODE void qspi_check_and_suspend_operations(void)
{
        ...
        if (qspi_active_op != NULL) {
                ...
                hw_clk_delay_usec(QSPI_GET_DEVICE_PARAM(idx, suspend_delay_us));
...
__RETAINED_CODE void qspi_int_reset_device(HW_QSPIC_ID id)
{
        ...
        /* Release teh FLASH/RAM from Power-Down mode to enable it to accept commands */
        flash_set_bus_mode(id, HW_QSPI_BUS_MODE_SINGLE);
...
#else
        // In flash builds, the booter has already applied all possible reset sequences, therefore
        // there is no reason to do it again. However, the memory has to get out from continuous
        // mode of operation (if enabled), otherwise, the read JEDEC ID will fail.
        flash_reset_continuous_mode(id, HW_QSPI_BREAK_SEQ_SIZE_2B);
#endif
...
__STATIC_FORCEINLINE void calculate_cs_idle_clk_cycles(HW_QSPIC_ID id, sys_clk_t sys_clk,
...
__RETAINED_CODE void qspi_int_configure(HW_QSPIC_ID id)
{
        ...
        qspi_init_config.read_instruction.dummy_count = QSPI_GET_DEVICE_PARAM(idx, get_dummy_bytes)(id, sysclk_RC32);
...
#       endif
                ...
                if (autodetect[qspi_control_idx]) {
                        qspi_is_device_present[qspi_control_idx] = qspi_read_flash_jedec_id(id, &manufacturer_id, &device_type, &device_density);
                        ...
                        for (uint8_t i = 0; i < ARRAY_LENGTH(qspi_memory_config_table); i++) {
                                if ((qspi_memory_config_table[i]->manufacturer_id == manufacturer_id) &&
                                        ...
                                        (qspi_density_is_equal(qspi_memory_config_table[i]->device_density, device_density))) {
                                        flash_config_init = qspi_memory_config_table[i];
                                        break;
                                }
                        }
...
{
        hw_qspi_set_read_pipe_clock_delay(id, sys_clk == sysclk_PLL96 ?
                                          READ_PIPE_DELAY_1V2 : READ_PIPE_DELAY_0V9);
}
