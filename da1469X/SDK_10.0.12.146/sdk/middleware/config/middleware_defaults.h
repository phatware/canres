...
#ifndef dg_config_ENABLE_RC32K_CALIBRATION
#define dg_config_ENABLE_RC32K_CALIBRATION      (1)
#endif
...
#error "Configuration option dg_configHAPTIC_ADAPTER also requires either the dg_configUSE_HW_LRA or the dg_configUSE_HW_ERM configuration options to be set."
#endif
#endif
...
/* For the calibration of the RC32K, the RCX or the RF we need to enable SNC, the GPADC and SYS_ADC as well */
#if (dg_configRF_ENABLE_RECALIBRATION || ((dg_config_ENABLE_RC32K_CALIBRATION || dg_configUSE_LP_CLK == LP_CLK_RCX) && defined(OS_FREERTOS)))
...
#endif /* (dg_configRF_ENABLE_RECALIBRATION || dg_config_ENABLE_RC32K_CALIBRATION || (dg_configUSE_LP_CLK == LP_CLK_RCX && defined(OS_FREERTOS))) */

/**
 *
 * \brief RTC-to-PDC event period, in 10ms
 *
 * The dg_configRTC_PDC_EVENT_PERIOD defines the RTC-to-PDC event period that will be used by the RTC event controller.
 * This is required when any of following conditions is met:
 *
 *  - RC32K calibration is enabled.
 *  - RF calibration is enabled.
 *  - RCX is set as the low power clock
 *
 * \bsp_default_note{\bsp_config_option_expert_only,}
 *
 */
#if (dg_configRF_ENABLE_RECALIBRATION || ((dg_config_ENABLE_RC32K_CALIBRATION || dg_configUSE_LP_CLK == LP_CLK_RCX) && defined(OS_FREERTOS)))
#ifndef dg_configRTC_PDC_EVENT_PERIOD
#define dg_configRTC_PDC_EVENT_PERIOD   (100)   // is in multiples of 10ms
#endif
#else
...
/**
 *
 * \brief RC calibration polling interval counter
 *
 * dg_configRC_CAL_POLL_INT defines the number of RTC-to-PDC events that SYS_RCX ucode should wait before executing its code.
 *
 * \bsp_default_note{\bsp_config_option_expert_only,}
 *
 */
#if (dg_configRF_ENABLE_RECALIBRATION || dg_config_ENABLE_RC32K_CALIBRATION || (dg_configUSE_LP_CLK == LP_CLK_RCX && defined(OS_FREERTOS)))
#ifndef dg_configRC_CAL_POLL_INT
#define dg_configRC_CAL_POLL_INT       (1)     // in dg_configRTC_PDC_EVENT_PERIOD * 10ms - calibration interval = 1sec
#endif
#endif
 ...
 *
 */
#if (dg_configRF_ENABLE_RECALIBRATION || dg_config_ENABLE_RC32K_CALIBRATION)
 ...
 */
#if (dg_configUSE_LP_CLK == LP_CLK_RCX && defined(OS_FREERTOS) && dg_configUSE_HW_RTC)
