                ...
                while (*uart) {
                        while (hw_uart_cts_getf(ad_uart_ctrl_conf->id)) {
                                ...
                                if ((pending_requests & mask) == 0) {
                                        uint32_t new_bits = 0;
                                        OS_TASK_NOTIFY_WAIT(0, ~0, &new_bits, OS_MS_2_TICKS(10000));
                        ...
                        *uart = (ad_uart_handle_t *)ad_uart_close(uart, true);
                        OS_EVENT_SIGNAL(console.read_finished);
                }
        }
}
...
void console_init(const ad_uart_controller_conf_t *ad_uart_ctrl_conf)
{
        ...
        OS_TASK_CREATE("console", console_task_fun, (void *)ad_uart_ctrl_conf,
                configMINIMAL_STACK_SIZE * OS_STACK_WORD_SIZE * 2, CONSOLE_TASK_PRIORITY, console.task);
