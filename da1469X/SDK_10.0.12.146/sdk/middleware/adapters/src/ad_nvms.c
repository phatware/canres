...
#define PARTITION_TABLE_ENTRY(start, size, id) \
        {(start) / (AD_FLASH_SECTOR_SIZE), (size) / (AD_FLASH_SECTOR_SIZE), (id), 0},
#define PARTITION(start, id, flags) \
        {(start) / (AD_FLASH_SECTOR_SIZE), 0, (id), (flags)},
#define PARTITION2(id, flags) \
        {(id##_START) / (AD_FLASH_SECTOR_SIZE), (id##_SIZE) / (AD_FLASH_SECTOR_SIZE), (id), (flags)},
...
size_t ad_nvms_get_size(nvms_t handle)
{
        ...
        if (part->driver->get_size) {
                ...
                return part->data.sector_count * AD_FLASH_SECTOR_SIZE;
        }
}
...
bool ad_nvms_no_cache_flushing(nvms_t handle, uint32_t base, uint32_t size)
{
        ...
        addr = part->data.start_sector * AD_FLASH_SECTOR_SIZE + base;
