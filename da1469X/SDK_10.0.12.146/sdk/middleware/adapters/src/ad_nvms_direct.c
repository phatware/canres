...
{
        void *p = OS_MALLOC_NORET(AD_FLASH_SECTOR_SIZE);
        if (p) {
...
static uint8_t flash_sector[AD_FLASH_SECTOR_SIZE];
...
{
        return part->data.start_sector * AD_FLASH_SECTOR_SIZE + addr;
}
...
{
        ad_flash_read(flash_addr, sec->buf, AD_FLASH_SECTOR_SIZE);
...
__STATIC_INLINE void flush_sector(cached_sector *sec, bool erase_cache)
{
        if (sec->in_use) {
                ...
                ad_flash_erase_region(sec->flash_address, AD_FLASH_SECTOR_SIZE);
                ad_flash_write(sec->flash_address, sec->buf, AD_FLASH_SECTOR_SIZE);
                if (erase_cache) {
                        /*
                                                                                ...
                                                                                uint32_t size)
{
        ...
        if (addr + size > part->data.sector_count * AD_FLASH_SECTOR_SIZE) {
                size = part->data.sector_count * AD_FLASH_SECTOR_SIZE - addr;
        }
        ...
        if (sector_buff.in_use) {
                ...
                uint32_t end = MIN(sector_buff.flash_address + AD_FLASH_SECTOR_SIZE,
                                                                read_address + size);
                                                                                ...
                                                                                uint32_t size)
{
        ...
        while (written < size) {
                sector_start = addr & ~(AD_FLASH_SECTOR_SIZE - 1);
                sector_offset = addr - sector_start;
                chunk_size = AD_FLASH_SECTOR_SIZE - sector_offset;
                ...
                if (addr == sector_start && chunk_size == AD_FLASH_SECTOR_SIZE) {
                        ad_flash_erase_region(part_addr(part, sector_start), AD_FLASH_SECTOR_SIZE);
                        ad_flash_write(part_addr(part, sector_start), buf, AD_FLASH_SECTOR_SIZE);
                } else {
                        /*
                                                                                ...
                                                                                const void **ptr)
{
        ...
        if (addr >= part->data.sector_count * AD_FLASH_SECTOR_SIZE) {
                size = 0;
                *ptr = NULL;
        } else {
                if (addr + size > part->data.sector_count * AD_FLASH_SECTOR_SIZE) {
