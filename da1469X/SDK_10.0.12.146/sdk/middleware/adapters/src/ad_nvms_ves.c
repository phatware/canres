...
#if ((AD_FLASH_SECTOR_SIZE) / (AD_NVMS_VES_CONTAINER_SIZE)) <= 256
...
{
        return (ves->start_sector + sector) * AD_FLASH_SECTOR_SIZE + container * ves->container_size;
}

__STATIC_INLINE uint32_t container_data_addr(ves_driver_data_t *ves, sec_ix_t sector,
                                                                con_ix_t container, uint8_t offset)
{
        return (ves->start_sector + sector) * AD_FLASH_SECTOR_SIZE + offset +
...
static void ves_init_sector(ves_driver_data_t *ves, sec_ix_t sector, bool check_dirty)
{
        ...
        if (erase_needed) {
                /* Erase whole sector */
                ad_flash_erase_region(addr, AD_FLASH_SECTOR_SIZE);
        }
...
void ves_init(struct partition_t *part)
{
        ...
        ves->containers_per_sector = AD_FLASH_SECTOR_SIZE / ves->container_size;
        ...
        ves->cat_size = (ves->container_data_size - 1 + ves->sector_count * AD_FLASH_SECTOR_SIZE) /
