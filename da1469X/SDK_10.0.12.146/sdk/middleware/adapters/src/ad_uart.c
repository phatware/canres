...
static int ad_uart_gpio_config(HW_UART_ID id, const ad_uart_io_conf_t *io, AD_IO_CONF_STATE state,
                                                                                bool is_ext_api)
{
        uint8_t ret;
        ...
        ret = ad_io_configure(&io->rx, size, io->voltage_level, state);
        if (ret != AD_IO_ERROR_NONE) {
                return ret;
        }
