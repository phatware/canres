 ...
 *
 * \deprecated This function is deprecated. User shall call hids_set_boot_mouse_input_report and
 * hids_notify_boot_mouse_input_report instead.
 */
static inline bool hids_set_boot_mouse_input_value(ble_service_t *svc, uint16_t length, const uint8_t *data);
static inline __attribute__((deprecated("This function is deprecated. User is advised to call hids_set_boot_mouse_input_report() "
                                        "and hids_notify_boot_mouse_input_report instead.")))
bool hids_set_boot_mouse_input_value(ble_service_t *svc, uint16_t length, const uint8_t *data)
{
        if (!hids_set_boot_mouse_input_report(svc, length, data)) {
                return false;
        }
        return hids_notify_boot_mouse_input_report(svc, length, data);
}
 ...
 */
static inline bool hids_set_boot_keyboard_input_value(ble_service_t *svc, uint16_t length, const uint8_t *data);
static inline __attribute__((deprecated("This function is deprecated. User is advised to call hids_set_boot_keyboard_input_report() "
                                        "and hids_notify_boot_keyboard_input_report() instead.")))
bool hids_set_boot_keyboard_input_value(ble_service_t *svc, uint16_t length, const uint8_t *data)
{
        if (!hids_set_boot_keyboard_input_report(svc, length, data)) {
                return false;
        }

        return hids_notify_boot_keyboard_input_report(svc, length, data);
}
 ...
 *
 */
static inline bool hids_set_report_value(ble_service_t *svc, hids_report_type_t type, uint8_t report_id,
                                         uint16_t length, const uint8_t *data);
static inline __attribute__((deprecated("This function is deprecated. User is advised to call hids_set_report() "
                                        "and hids_notify_input_report() instead.")))
bool hids_set_report_value(ble_service_t *svc, hids_report_type_t type, uint8_t report_id,
                           uint16_t length, const uint8_t *data)
{
        if (!hids_set_report(svc, type, report_id, length, data)) {
                return false;
        }
        return hids_notify_input_report(svc, report_id, length, data);
}
