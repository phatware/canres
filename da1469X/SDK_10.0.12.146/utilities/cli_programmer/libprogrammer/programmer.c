...
int write_otp_32(uint32_t address, const uint32_t *buf, uint32_t len)
{
        ...
        if (otp_same) {
                prog_print_err("Otp address 0x%x has same data...\n",
                        (chip_690_regs.otp_start_address + (address * 4 + i * 4)));
                free(read_buf);
                return ERR_PROG_OTP_SAME;
...
int prog_write_otp(uint32_t address, const uint32_t *buf, uint32_t len)
{
        if (strncmp(prog_chip_rev, CHIP_REV_690AB, CHIP_REV_STRLEN) == 0) {
                ...
                while (len > offset) {
                        ...
                        /* Maximum OTP write chunk size for protocol commands is set to 1 KB. */
                        if (chunk_size > PROTOCOL_MAX_OTP_CHUNK_SIZE) {
                                chunk_size = PROTOCOL_MAX_OTP_CHUNK_SIZE;
                        }
                        res = write_otp_32(address + offset, buf + offset, chunk_size);
                        /* return the error */
                        if (res != 0) {
                                return res;
                        }
                        prog_print_log("Programmed OTP to address:0x%08x offset: 0x%08x chunk size(words): 0x%08x\n",
                                chip_690_regs.otp_start_address + (address * 4), offset * 4, chunk_size);
                        offset += chunk_size;
                }
                return res;
        } else {
...
int prog_gdb_read_chip_rev(char *chip_rev)
{
        ...
        if ((strcmp(chip_rev, "252AB") == 0) || (strcmp(chip_rev, "308AA") == 0)) {
                strncpy(chip_rev, "690", CHIP_REV_STRLEN);
        }
