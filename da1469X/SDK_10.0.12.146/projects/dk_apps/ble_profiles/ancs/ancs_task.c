...
static notification_t *add_notification(uint32_t uid, const ancs_notification_data_t *data)
{
         ...
         */
        if ((queue_length(&notif_q) + 1) > CFG_NOTIF_QUEUE_MAX) {
...
#if CFG_APP_QUEUE_MAX
        /*
         * If a maximum limit for temporarily saved applications has been defined and reached,
         * remove the oldest added application.
         */
        if ((queue_length(&app_q) + 1) > CFG_APP_QUEUE_MAX) {
                application_t *oldest_app = queue_pop_front(&app_q);
                free_application(oldest_app);
        }
#endif
