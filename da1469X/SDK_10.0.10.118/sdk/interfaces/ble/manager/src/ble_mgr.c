...
__RETAINED static uint8_t skip_latency_mask;
...
void ble_mgr_event_queue_flush(void)
{
        ...
        if (OS_GET_CURRENT_TASK() == app_task) {
                uint32_t notif;
                OS_TASK_NOTIFY_WAIT(0, mainBIT_MANAGER_EVENT_QUEUE, &notif, OS_TASK_NOTIFY_NO_WAIT);
        }
#endif /* (BLE_MGR_USE_EVT_LIST == 0) */
}
...
OS_BASE_TYPE ble_mgr_event_queue_get(void *item, OS_TICK_TIME wait_ticks)
{
        ...
        if (mgr_if.evt_q == NULL) {
                ...
                OS_TASK_NOTIFY_WAIT(0, mainBIT_MANAGER_EVENT_QUEUE, &notif, wait_ticks);
