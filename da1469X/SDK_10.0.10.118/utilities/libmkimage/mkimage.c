...
static bool get_date(size_t size, const char *buffer, uint32_t *hdr_timestamp)
{
        const char *from;
        char *timestamp;
        struct tm tm;
        ...
        store32((uint8_t *) hdr_timestamp, (uint32_t) secs);
        free(timestamp);
                                                                ...
                                                                size_t out_size, uint8_t *out)
{
        ...
        if (!get_date(ver_size, (char *) ver, &header.timestamp)) {
                return MKIMAGE_STATUS_INVALID_DATA;
        }
